...
3.  ```如果只有一张表我们需要跟据这张表的一个字段的关系查询数据，比如我需要比前一天的日期的温度值大，这个时候我就需要这个表与自己的日期字段比较，这个时候我们可以用内连接的方式，然后在on这个字段后面加上筛选条件，比如on t1.date = t2.date + 1，但是注入日期的比较如果单纯的+1遇到月末就会漏数据，所以有个datediff这个函数可以传进去两个日期，返回差值```